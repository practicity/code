...
public class KafkaBridgeSelectedEntities extends DisplayEntity {
...

    {
        ...
        entitiesToMonitor.setRequired(true);
    ...
    private class PollTarget extends ProcessTarget {
        ...
        public String getDescription() {
            return KafkaBridgeSelectedEntities.this.getName() + "-Poll";
        }
        ...
        public void process() {
            ...
            if (entitiesToMonitor.getValue() != null) {
                for (DisplayEntity entity : entitiesToMonitor.getValue()) {
    ...
    // =========================================================================
    // KAFKA SEND WITH ERROR HANDLING
    // =========================================================================
    ...
    private final Callback sendCallback = new Callback() {
        ...
        public void onCompletion(RecordMetadata metadata, Exception exception) {
            if (exception != null) {
                ...
                if (sendErrorCount == getMaxLogErrors()) {
                    logWarn("Suppressing further Kafka send error messages");
                }
            }
        }
    };
    ...
    private void buildQueueJson(StringBuilder sb, Queue queue, double simTime) {
        try {
            appendLong(sb, "length", (long) queue.getQueueLength(simTime));
        } catch (Exception e) {
            logMetricError("Queue.length", queue.getName(), e);
        }

        try {
            appendDecimal(sb, "avgLength", queue.getQueueLengthAverage(simTime));
        } catch (Exception e) {
            logMetricError("Queue.avgLength", queue.getName(), e);
        }

        try {
            // FIXED: was getMaxQueueLength() — correct is getQueueLengthMaximum(simTime)
            appendLong(sb, "maxLength", (long) queue.getQueueLengthMaximum(simTime));
        } catch (Exception e) {
            logMetricError("Queue.maxLength", queue.getName(), e);
        }

        try {
            appendLong(sb, "minLength", (long) queue.getQueueLengthMinimum(simTime));
        } catch (Exception e) {
            logMetricError("Queue.minLength", queue.getName(), e);
        }

        try {
            appendLong(sb, "numberAdded", queue.getNumberAdded(simTime));
        } catch (Exception e) {
            logMetricError("Queue.numberAdded", queue.getName(), e);
        }

        try {
            appendLong(sb, "numberProcessed", queue.getNumberProcessed(simTime));
        } catch (Exception e) {
            logMetricError("Queue.numberProcessed", queue.getName(), e);
        }

        try {
            appendLong(sb, "queuePosition", queue.getQueuePosition(simTime));
        } catch (Exception e) {
            logMetricError("Queue.queuePosition", queue.getName(), e);
        }

        try {
            appendTime(sb, "avgWait", queue.getAverageQueueTime(simTime));
        } catch (Exception e) {
            logMetricError("Queue.avgWait", queue.getName(), e);
        }

        try {
            appendLong(sb, "numberReneged", queue.getNumberReneged(simTime));
        } catch (Exception e) {
            logMetricError("Queue.numberReneged", queue.getName(), e);
        }
    }
    ...
    private void buildServerJson(StringBuilder sb, Server server, double simTime) {
        ...
        // Utilization - try multiple state names for robustness
        try {
            double totalTime = Math.max(simTime, 0.001);
            double workingTime = 0;

            for (String stateName : new String[]{"Working", "working", "Processing", "Busy"}) {
                try {
        ...
        // getNumberInProgress requires simTime argument
        try {
    ...
    private void sendSimulationEvent(String eventType) {
        ...
        if (entitiesToMonitor.getValue() != null && !entitiesToMonitor.getValue().isEmpty()) {
    ...
    // =========================================================================
    // LOGGING HELPERS - prefixed to avoid collision with Entity.error()
    // =========================================================================
